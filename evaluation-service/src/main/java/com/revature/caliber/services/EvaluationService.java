...
public class EvaluationService {
	...
	public Map<Integer, List<Grade>> findGradesByWeek(Integer batchId, Integer week) {
		...
		for (Grade grade : grades) {
			Integer key = grade.getTrainee();
			if (table.containsKey(grade.getTrainee())) {
	...
	public List<Grade> findByTrainee(Integer traineeId) {
		return gradeRepository.findByTraineeAndScoreGreaterThan(traineeId, ZERO);
	}
	...
	public void checkIfTraineeShouldBeFlagged(Note note) {
		if (note != null && note.getType().equals(NoteType.QC_TRAINEE)) {
			...
			List<Note> notes = noteRepository
					.findByTraineeAndTypeOrderByWeekAsc(note.getTrainee(), NoteType.QC_TRAINEE);
			...
			try {
				...
				if (flagged) {
					...
					/* TODO this logic needs to be in training-service.
					 
					 Trainee trainee = traineeRepository.findOne(note.getTrainee());
	...
	public List<Note> findBatchNotes(Integer batchId, Integer week) {
		...
		return noteRepository.findByBatchAndWeekAndType(batchId, week.shortValue(), NoteType.BATCH);
	}
	...
	public List<Note> findIndividualNotes(Integer batchId, Integer week) {
		...
		return noteRepository.findByBatchAndWeekAndType(batchId, week.shortValue(), NoteType.TRAINEE);
	}
	...
	public Note findTraineeNote(Integer traineeId, Integer week) {
		return noteRepository.findByTraineeAndWeekAndType(traineeId, week.shortValue(), NoteType.TRAINEE);
	}
	...
	public Note findQCTraineeNote(Integer traineeId, Integer week) {
		return noteRepository.findByTraineeAndWeekAndType(traineeId, week.shortValue(), NoteType.QC_TRAINEE);
	}
	...
	public Note findQCBatchNotes(Integer batchId, Integer week) {
		...
		List<Note> notes = noteRepository.findByBatchAndWeekAndType(batchId, week.shortValue(),
				NoteType.QC_BATCH);
	...
	public List<Note> findQCIndividualNotes(Integer traineeId, Integer week) {
		...
		return noteRepository.findByTraineeAndWeekAndTypeOrderByWeekAsc(traineeId, week.shortValue(),
				NoteType.QC_TRAINEE);
	}
	...
	public List<Note> findAllBatchNotes(Integer batchId, Integer week) {
		log.debug(FINDING_WEEK + week + " batch notes for batch: " + batchId);
		return noteRepository.findByBatchAndWeekAndTypeOrderByWeekAsc(batchId, week.shortValue(), NoteType.BATCH);
	}
	...
	public List<Note> findAllQCTraineeNotes(Integer batchId, Integer week) {
		...
		return noteRepository
				.findByBatchAndWeekAndTypeOrderByWeekAsc(batchId, week.shortValue(), NoteType.QC_TRAINEE);
	}
	...
	public List<Note> findAllIndividualNotesOverall(Integer traineeId) {
		...
		return noteRepository.findByTraineeAndTypeOrderByWeekAsc(traineeId, NoteType.TRAINEE);
	}
	...
	public List<Note> findAllQCTraineeOverallNotes(Integer traineeId) {
		...
		return noteRepository.findByTraineeAndTypeOrderByWeekAsc(traineeId, NoteType.QC_TRAINEE);
	}
	 ...
	 * @param batch
	 */
	public void calculateAverage(Integer week, Integer batch) {
		if (batch != null) {
			Note notes = noteRepository.findByBatchAndWeekAndTypeOrderByWeekDesc(batch, week.shortValue(),
					NoteType.QC_BATCH);
			...
			List<Note> traineeNoteList = noteRepository
					.findByBatchAndWeekAndTypeOrderByWeekAsc(batch, week.shortValue(),
							NoteType.QC_TRAINEE);
	...
	public List<List<Note>> findAllQCTraineeNotesForAllWeeks(Integer batchId) {
		log.debug("Find All QC Trainee Notes");
		List<Note> notes = noteRepository.findByBatchAndTypeOrderByWeekAsc(batchId, NoteType.QC_TRAINEE);
		ArrayList<List<Note>> noteFormatted2d = new ArrayList<>();
		// TODO Refactor this to be isolated to the Evaluation-service and not training-service
	...
	public List<Note> findAllQCBatchNotes(Integer batchId) {
		...
		return noteRepository.findByBatchAndTypeOrderByWeekAsc(batchId, NoteType.QC_BATCH);
	}
